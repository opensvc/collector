...
response.menu = [
  [DIV(T('Navigation'), _class="menu16"), False, '',
    ...
    [T('Views'), False, '',
         [
             [DIV(T('Dashboard'), DIV(T("Current issues on nodes and services")), _class='alert48', _style='background-size:40px 40px'), display('view-dashboard'),
             ...
             [DIV(T('Nodes'), DIV(T("Technical and organizational info")), _class='node48', _style='background-size:40px 40px'), display('view-nodes'),
              URL(request.application,'nodes','nodes')],
             [DIV(T('Actions'), DIV(T("Service actions log")), _class='action48', _style='background-size:40px 40px'), display('view-actions'),
             ...
             [DIV(T('Saves'), DIV(T("Backup servers aggregated index")), _class='backup48', _style='background-size:40px 40px'), display('view-saves'),
              URL(request.application,'saves','saves')],
             [DIV(T('Disks'), DIV(T("All known disks with their array information")), _class='disk48', _style='background-size:40px 40px'), display('view-disks'),
              URL(request.application,'disks','disks')],
             [DIV(T('Storage Arrays'), DIV(T("Symmetrix detailled information")), _class='disk48', _style='background-size:40px 40px'), display('view-arrays'),
    ...
    [T('Compliance'), False, '',
         [
             ...
             [DIV(T('Log'), DIV(T("All configuration checks and fixes")), _class='log48', _style='background-size:40px 40px'), display('comp-log'),
              URL(request.application,'compliance','comp_log')],
             [DIV(T('Modulesets'), DIV(T("Search-optimized modules grouping view")), _class='action48', _style='background-size:40px 40px'), display('comp-modsets'),
              URL(request.application,'compliance','comp_modules')],
             [DIV(T('Rulesets'), DIV(T("Search-optimized target configurations view")), _class='comp48', _style='background-size:40px 40px'), display('comp-rsets'),
             ...
             [DIV(T('Node rulesets'), DIV(T("Mass attach and detach tool")), _class='rsetattach48', _style='background-size:40px 40px'), display('comp-node-rset'),
              URL(request.application,'compliance','comp_rulesets_nodes_attachment')],
             [DIV(T('Node modulesets'), DIV(T("Mass attach and detach tool")), _class='modsetattach48', _style='background-size:40px 40px'), display('comp-node-modset'),
              URL(request.application,'compliance','comp_modulesets_nodes')],
             [DIV(T('Service rulesets'), DIV(T("Mass attach and detach tool")), _class='rsetattach48', _style='background-size:40px 40px'), display('comp-svc-rset'),
              URL(request.application,'compliance','comp_rulesets_services_attachment')],
             [DIV(T('Service modulesets'), DIV(T("Mass attach and detach tool")), _class='modsetattach48', _style='background-size:40px 40px'), display('comp-svc-modset'),
              URL(request.application,'compliance','comp_modulesets_services')],
         ]
    ],
    [T('Statistics'), False, '',
         [
             [DIV(T('Reports'), DIV(T("User-defined reports")), _class='stats48', _style='background-size:40px 40px'), display('stat-reports'),
              URL(request.application,'charts','reports')],
             [DIV(T('Site'), DIV(T("Pre-defined site report")), _class='stats48', _style='background-size:40px 40px'), display('stat-site'),
              URL(request.application,'stats','stats')],
             [DIV(T('Compare'), DIV(T("Compare node and service subsets evolution")), _class='stats48', _style='background-size:40px 40px'), display('stat-compare'),
              URL(request.application,'stats','compare')],
             [DIV(T('Os lifecycle'), DIV(T("Operating systems dispatch evolution")), _class='stats48', _style='background-size:40px 40px'), display('stat-os-lifecycle'),
              URL(request.application,'lifecycle','lifecycle_os')],
             [DIV(T('Availability'), DIV(T("Service outage timelines")), _class='avail48', _style='background-size:40px 40px'), display('stat-avail'),
    ...
    [T('Administration'), False, '',
         [
             [DIV(T('Users'), DIV(T("Users and group administration")), _class='guys48', _style='background-size:40px 40px'), display('adm-usr', privs=['Manager']),
              URL(request.application,'users','users')],
             [DIV(T('Log'), DIV(T("Collector events log")), _class='log48', _style='background-size:40px 40px'), display('adm-log'),
             ...
             [DIV(T('Batchs'), DIV(T("Collector janitoring batchs")), _class='action48', _style='background-size:40px 40px'), display('adm-batchs', privs=['Manager']),
              URL(request.application,'batchs','batchs')],
             [DIV(T('Billing'), DIV(T("Licensing tokens count and dispatch")), _class='billing48', _style='background-size:40px 40px'), display('adm-bill', privs=['Manager']),
             ...
             [DIV(T('Filters'), DIV(T("Create new filtersets")), _class='filter48', _style='background-size:40px 40px'), display('adm-filters'),
             ...
             [DIV(T('Metrics'), DIV(T("Design sql requests to embed in charts")), _class='stats48', _style='background-size:40px 40px'), display('adm-metrics'),
              URL(request.application,'charts','metrics_admin')],
             [DIV(T('Charts'), DIV(T("Design charts to embed in reports")), _class='stats48', _style='background-size:40px 40px'), display('adm-charts'),
              URL(request.application,'charts','charts_admin')],
             [DIV(T('Reports'), DIV(T("Design custom reports")), _class='stats48', _style='background-size:40px 40px'), display('adm-reports'),
    ...
    def serialize(self, data, level=0):
        ...
        if level == 1:
            return DIV(
                     ...
                     ul,
                     _class="menu_top",
                     _onclick="""$(this).children("div").toggle("fade", 200);$("#search").focus()"""
                   )
        return ul
