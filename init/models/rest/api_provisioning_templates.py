...
class rest_delete_provisioning_template_responsible(rest_delete_handler):
    ...
    def handler(self, tpl_id, group_id, **vars):
        ...
        fmt = "Provisioning template %(tpl_id)s responsibility to group %(group_id)s removed"
        d = dict(tpl_id=str(tpl_id), group_id=str(group_id))

        row = db(q).select().first()
        if row is None:
            return dict(info="Provisioning template %(tpl_id)s responsibility to group %(group_id)s already removed" % d)
...
class rest_post_provisioning_template_responsible(rest_post_handler):
    ...
    def handler(self, tpl_id, group_id, **vars):
        ...
        row = db(q).select().first()
        if row is not None:
            return dict(info="Provisioning template %(tpl_id)s responsibility to group %(group_id)s already added" % d)
...
class rest_delete_provisioning_template_publication(rest_delete_handler):
    ...
    def handler(self, tpl_id, group_id, **vars):
        ...
        fmt = "Provisioning template %(tpl_id)s publication to group %(group_id)s removed"
        d = dict(tpl_id=str(tpl_id), group_id=str(group_id))

        row = db(q).select().first()
        if row is None:
            return dict(info="Provisioning template %(tpl_id)s publication to group %(group_id)s already removed" % d)
...
class rest_post_provisioning_template_publication(rest_post_handler):
    ...
    def handler(self, tpl_id, group_id, **vars):
        ...
        row = db(q).select().first()
        if row is not None:
            return dict(info="Provisioning template %(tpl_id)s publication to group %(group_id)s already added" % d)
